...
class GoalInfo extends React.Component {
  ...
  submitInfo() {
    ...
    axios
      .post('/retire/goals', userInfoToSubmit)
      .then((res) => {
  ...
  render() {
    ...
    let slideOptions = [
      <div className="tab tab0 form-group">
        Optional: Input additional information about your goals to calculate your retirement
        precisely!
      </div>,
      <div className="tab tab1 form-group">
        Tell us about your planned number of financial contributors
        <p>
          <input
            type="number"
      ...
      <div className="tab tab2 form-group">
        How many children are in your household/you planning to have in your household?
        <p>
          <input
            type="number"
            className="form-control"
            ...
            max="20"
            onChange={(event) => {
              this.setState({ numberOfKids: event.target.value });
            }}
          />
        </p>
      </div>,
      <div className="tab tab3 form-group">
        Post retirement, how much do you plan on spending on travel annually?
        <p>
          <input
            type="number"
            className="form-control"
      ...
      <div className="tab tab4 form-group">
        Post retirement, how much do you plan on spending on your hobbies annually?
        <p>
          <input
            type="number"
            className="form-control"
            value={this.state.hobbySpending}
            min="0"
            max="10000000"
            onChange={(event) => {
              this.setState({ hobbySpending: event.target.value });
            }}
          />
        </p>
      </div>,
      <div className="tab tab5 form-group">
        Last question, post retirement, how much do you plan on spending on luxuries annually?
        <p>
          <input
            type="number"
            className="form-control"
            value={this.state.luxurySpending}
            min="0"
            onChange={(event) => {
              this.setState({ luxurySpending: event.target.value });
            }}
          />
        </p>
      </div>
